...
def iterate():
    ...
    nH2O=numpy.zeros((binCount,binCount))
    fH2O=numpy.zeros((binCount,binCount))
    ...
    # Compute radial vectors
            if ts.frame <= 0: # for belly simulations there is no need to compute these every frame.
    ...
    # Define 3 axes of solute
            ...
            for a in H2OCoord.residues:
                ...
                rWat = (a.atoms[1].position + d * ((a.atoms[1].position - a.atoms[0].position)/1.1)) - rCen # selecting C1 in CL3 which is index 1 and H1 (0), to make new vector to center of volume exclusion from solute CoM.
                for i in range(3):
                    while rWat[i] < -hdims[i]:
                        rWat[i]+=dims[i]
                    while rWat[i] > hdims[i]:
                        rWat[i]-=dims[i]
                rWat=numpy.dot(axes,rWat) # project solvent position onto solute axes
    # Calculate x,r bin
                x=rWat[0]+hrMax
...
#                                force_var = numpy.dot( solvAtom_force_vec, ljdr) # project force from solvent ATOMs onto LJ -- LJ vector
                                ###
                                #gr3Df2py.calc_force(len(a.atoms), n_atoms, atom_type_index, n_types, nb_parm_index, ionsCoord.atoms[0].index, ionsCoord.atoms[0].position, a.atoms.positions, dims, hdims, lj_a_coeff, lj_b_coeff, ljdr, force_var)
                                gr3Df2py.calc_force( atom_type_index, n_types, nb_parm_index, ionsCoord.atoms[0].index, ionsCoord.atoms[0].position, a.atoms.positions, dims, hdims, lj_a_coeff, lj_b_coeff, ljdr, force_var, len(a.atoms), n_atoms )
                                fH2O[ix][ir] += force_var
#
    dxH2O=1/(2*pi*binSize**2/dims[0]/dims[1]/dims[2]*len(H2OCoord.residues)*total_frames) # normalize
    outFile = open(outname+".cyl.gr3", 'w')
    outFile.write("# 1: Longitudinal Axis (x)\n")
    outFile.write("# 2: r Axis\n")
    outFile.write("# 3: g(r)\n")
    outFile.write("# 4: f(r)\n")
    for ir in range(binCount/2):
        for ix in range(binCount):
            ...
            r = (ir+0.5) * binSize
            outFile.write("{:7.3f} {:7.3f} {:18.12f} {:18.12f}\n".format((ix+0.5)*binSize-hrMax, (ir+0.5)*binSize, nH2O[ix][ir]*dxH2O/r, fH2O[ix][ir]/nH2O[ix][ir])) #,pH2O[i][j][k][0],pH2O[i][j][k][1],pH2O[i][j][k][2]))
    outFile.close()
####
